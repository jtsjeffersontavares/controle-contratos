...
uses_postgres = DB_ENGINE in {"postgres", "postgresql"} or (
    ...
    and parsed_database_url.scheme
    in {"postgres", "postgresql", "postgresql+psycopg", "postgresql+psycopg2"}
)

if uses_postgres:
    if parsed_database_url is not None:
        database_name = parsed_database_url.path.lstrip("/") or os.getenv(
            "POSTGRES_DB", "gestao_contratos"
        )
        database_user = parsed_database_url.username or os.getenv(
            "POSTGRES_USER", "gestao_contratos"
        )
        database_password = parsed_database_url.password or os.getenv(
            "POSTGRES_PASSWORD", ""
        )
        database_host = parsed_database_url.hostname or os.getenv("POSTGRES_HOST", "db")
        database_port = str(
            parsed_database_url.port or os.getenv("POSTGRES_PORT", "5432")
        )
    else:
...
SYSTEM_ORGANIZATION = os.getenv(
    "SYSTEM_ORGANIZATION", "Subdiretoria de Apoio Administrativo"
)
