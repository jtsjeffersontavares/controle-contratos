...
def map_supply_status(status_value, delivered_value, deadline_value):
    ...
    if delivered in {"SIM", "ENTREGUE", "S"} or status in {
        "ENTREGUE",
        "CONCLUIDA",
        "CONCLUIDO",
    }:
...
def preview_workbook(source, sheet_name="Planilha1") -> dict:
    ...
    for excel_row, values in enumerate(matrix[1:], start=2):
        ...
        if not normalized_row["OM_DESTINO"]:
            row_warnings.append(
                "OM destino não informada; a linha não gerará ordem de fornecimento."
            )
        ...
        signature = (
            normalized_row["CONTRATO"],
            normalized_row["ITEM_PREGAO"],
            normalized_row["COD_TDV"],
            normalized_row["EMPENHO"],
            normalized_row["OM_DESTINO"],
            normalized_row["OFICIO_OF"],
        )
        if signature in seen_signatures:
            row_warnings.append(
                "Possível linha duplicada, com a mesma combinação de contrato, item, empenho e destino."
            )
    ...
    summary = {
        ...
        "total_value": str(
            sum((Decimal(row["VALOR_TOTAL"]) for row in rows), Decimal("0"))
        ),
    }
    return {
...
def _person(text: str):
    ...
    if not text:
        return None
    match = re.match(
        r"^(CEL|TC|MAJ|CAP|1T|2T|ASP|SO|1S|2S|3S|CB|S1|S2)\s+(.+)$",
        text,
        flags=re.IGNORECASE,
    )
    rank, name = (
        (match.group(1).upper(), match.group(2).strip()) if match else ("", text)
    )
...
def _procurement(number: str, reference_org=None):
    ...
    return Procurement.objects.create(
        number=number, requesting_organization=reference_org
    )
...
def import_preview(preview: dict, actor=None, filename="planilha.xlsx") -> dict:
    if preview.get("summary", {}).get("errors"):
        raise ValueError(
            "A prévia contém erros impeditivos. Corrija a planilha antes de confirmar."
        )
    ...
    for contract_number, contract_rows in grouped_contracts.items():
        ...
        total_value = sum(
            (Decimal(row["VALOR_TOTAL"]) for row in contract_rows), Decimal("0")
        )
        nomenclatures = []
        ...
        contract, created = Contract.objects.update_or_create(
            ...
            defaults={
                ...
                "status": map_contract_status(
                    first["STATUS"] or first["STATUS_VIGENCIA"]
                ),
        ...
        for key, item_rows in item_groups.items():
            sample = item_rows[0]
            quantity = sum(
                (Decimal(row["QTD_EMPENHADO"]) for row in item_rows), Decimal("0")
            )
            procurement_item = None
            if procurement:
                procurement_item, procurement_item_created = (
                    ProcurementItem.objects.update_or_create(
                        procurement=procurement,
                ...
                counters[
                    (
                        "procurement_items_created"
                        if procurement_item_created
                        else "procurement_items_updated"
                    )
                ] += 1
        ...
        for row in contract_rows:
            if row["OM_DESTINO"]:
                delivery_totals[(_item_key(row), row["OM_DESTINO"])] += Decimal(
                    row["QTD_EMPENHADO"]
                )
        ...
        for (item_key, commitment_number), commitment_rows in commitment_groups.items():
            ...
            quantity = sum(
                (Decimal(row["QTD_EMPENHADO"]) for row in commitment_rows), Decimal("0")
            )
            value = sum(
                (Decimal(row["VALOR_TOTAL"]) for row in commitment_rows), Decimal("0")
            )
        ...
        for row in contract_rows:
            ...
            official_reference = (
                row["OFICIO_OF"] or f'IMPORTAÇÃO LINHA {row["EXCEL_ROW"]}'
            )
            status = map_supply_status(
                row["STATUS_ENTREGA"], row["ENTREGUES"], row["PRAZO_ENTREGA"]
            )
            ...
            if status == SupplyOrder.Status.COMPLETED and delivery_date:
                ...
                counters[
                    "deliveries_created" if delivery_created else "deliveries_updated"
                ] += 1
