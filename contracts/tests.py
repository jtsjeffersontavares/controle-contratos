...
class XlsxUtilityTests(TestCase):
    def test_write_and_read_xlsx(self):
        content = write_simple_xlsx(
            ["CONTRATO", "EMPRESA"], [["001/2026", "EMPRESA TESTE"]], "Planilha1"
        )
...
class ImportTests(TestCase):
    ...
    def workbook(self):
        ...
        return SimpleUploadedFile(
            "planilha.xlsx", write_simple_xlsx(headers, [row], "Planilha1")
        )
    ...
    def test_preview_rejects_missing_required_columns(self):
        file = SimpleUploadedFile(
            "invalida.xlsx", write_simple_xlsx(["PAG"], [["x"]], "Planilha1")
        )
    ...
    def test_audit_log_is_created_and_list_page_renders(self):
        ...
        self.assertTrue(
            AuditLog.objects.filter(
                action=AuditLog.Action.CREATE, model_name="Empresa"
            ).exists()
        )
...
class ContractModelTests(TestCase):
    ...
    def test_contract_form_renders_saved_dates_in_html5_format(self):
        contract = Contract.objects.create(
            number="005/FORM/2026",
            supplier=self.supplier,
            managing_organization=self.org,
            signature_date=date(2024, 1, 15),
...
class OrganizationFieldTests(TestCase):
    def test_organization_accepts_address_and_cep(self):
        org = Organization.objects.create(
            acronym="OM-END",
            name="Organização Endereço",
            city="Brasília",
            state="DF",
            address="Quadra 1, Bloco A, s/n",
            cep="70630-010",
        )

        self.assertEqual(org.address, "Quadra 1, Bloco A, s/n")
        self.assertEqual(org.cep, "70630-010")
...
class ViewAndPermissionTests(TestCase):
        ...
        with tempfile.TemporaryDirectory() as media_dir:
            with override_settings(MEDIA_ROOT=Path(media_dir)):
                ...
                document = Document.objects.create(
                    contract=contract,
                    title="Teste",
                    file=SimpleUploadedFile("teste.txt", b"conteudo protegido"),
                )
                ...
                self.assertEqual(response.status_code, 200)
                self.assertEqual(
                    b"".join(response.streaming_content), b"conteudo protegido"
                )
    ...
    def test_report_exports(self):
        ...
        procurement = Procurement.objects.create(
            number="91000/2026", requesting_organization=org
        )
        ...
        Contract.objects.create(
            number="003/RELATORIO/2026",
            supplier=supplier,
            managing_organization=org,
            current_value=Decimal("12345.67"),
            initial_value=Decimal("12345.67"),
        ...
        parsed = read_xlsx_sheet(
            SimpleUploadedFile("relatorio.xlsx", xlsx_response.content), "Contratos"
        )
        ...
        procurement_parsed = read_xlsx_sheet(
            SimpleUploadedFile("pregoes.xlsx", procurement_xlsx_response.content),
            "Pregoes",
        )
        ...
        self.assertIn(
            "91000/2026", procurement_csv_response.content.decode("utf-8-sig")
        )

        template_response = self.client.get(reverse("import_template"))
        self.assertEqual(template_response.status_code, 200)
        template_rows = read_xlsx_sheet(
            SimpleUploadedFile("modelo.xlsx", template_response.content), "Planilha1"
        )
    ...
    def test_dashboard_counts_closed_contracts_and_excludes_them_from_upcoming(self):
        ...
        expiring_contract = Contract.objects.create(
            number="007/DASHBOARD/2026",
            supplier=supplier,
            managing_organization=org,
            status=Contract.Status.ACTIVE,
            current_value=Decimal("200"),
            initial_value=Decimal("200"),
            end_date=timezone.localdate() + timedelta(days=30),
        )
    ...
    def test_main_pages_render(self):
        ...
        urls = [
            ...
            reverse("contract_detail", args=[contract.pk]),
            reverse("procurement_list"),
            reverse("procurement_detail", args=[procurement.pk]),
            reverse("supplier_list"),
            reverse("organization_list"),
            reverse("person_list"),
            reverse("commitment_list"),
            reverse("supplyorder_list"),
            ...
            reverse("procurement_create"),
            reverse("procurement_item_create") + f"?procurement={procurement.pk}",
    ...
    def test_contract_and_procurement_edit_forms_hide_removed_fields(self):
        ...
        procurement = Procurement.objects.create(
            number="90002/2026", requesting_organization=org
        )
        contract = Contract.objects.create(
        ...
        contract_response = self.client.get(
            reverse("contract_update", args=[contract.pk])
        )
        ...
        procurement_response = self.client.get(
            reverse("procurement_update", args=[procurement.pk])
        )
    ...
    def test_procurement_detail_shows_linked_items_per_contract_and_hides_om_line(self):
        ...
        procurement = Procurement.objects.create(
            number="90146/CAE/2025", requesting_organization=org
        )
    ...
    def test_contract_update_blocks_procurement_change_when_has_items(self):
        ...
        self.assertContains(
            get_response, "Pregão bloqueado: já existem itens vinculados ao contrato."
        )
    ...
    def test_supply_order_edit_filters_items_by_contract_and_shows_procurement_locations(
        self,
    ):
        self.client.login(username="gestor", password="SenhaForte123!")
        ...
        ProcurementItemDelivery.objects.create(
            item=procurement_item, destination=destination, quantity=Decimal("2")
        )
        ...
        post_response = self.client.post(
            reverse("supplyorder_update", args=[order.pk]),
            {
    ...
    def test_contract_detail_shows_procurement_edit_rule_message(self):
        ...
        response_without_items = self.client.get(
            reverse("contract_detail", args=[contract.pk])
        )
        self.assertEqual(response_without_items.status_code, 200)
        self.assertContains(
            response_without_items,
            "Pregão pode ser alterado até o primeiro item contratual ser cadastrado.",
        )
        ...
        response_with_items = self.client.get(
            reverse("contract_detail", args=[contract.pk])
        )
        self.assertEqual(response_with_items.status_code, 200)
        self.assertContains(
            response_with_items,
            "Pregão bloqueado para edição: já existe item contratual cadastrado.",
        )
    ...
    def test_contract_order_items_endpoint_returns_only_contract_items(self):
        ...
        ProcurementItemDelivery.objects.create(
            item=procurement_item, destination=dest, quantity=Decimal("2")
        )
        ...
        response = self.client.get(
            reverse("contract_order_items", args=[contract_a.pk])
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_procurement_item_form_rejects_planned_quantity_above_item_quantity(self):
        ...
        response = self.client.post(
            reverse("procurement_item_create"),
            {
                "procurement": procurement.pk,
                "item_number": "1",
                "code": "COD-1",
