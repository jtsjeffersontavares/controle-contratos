...
class DashboardView(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        orders = list(
            SupplyOrder.objects.select_related("contract", "destination").all()
        )
        ...
        total_value = sum(
            (contract.current_value for contract in contracts), Decimal("0")
        )
        total_order_qty = sum((order.quantity for order in orders), Decimal("0"))
        delivered_qty = sum(
            (order.delivered_quantity for order in orders), Decimal("0")
        )
        delivery_percent = (
            (delivered_qty * Decimal("100") / total_order_qty)
            if total_order_qty
            else Decimal("0")
        )
        ...
        max_destination = max(
            (row["total"] or 0 for row in top_destinations), default=1
        )
        for row in top_destinations:
            row["bar_percent"] = (
                float((row["total"] or 0) * 100 / max_destination)
                if max_destination
                else 0
            )
        upcoming = sorted(
        ...
        context.update(
            {
                ...
                "open_paai": AdministrativeProcess.objects.exclude(
                    status=AdministrativeProcess.Status.CLOSED
                ).count(),
                ...
                "status_cards": [
                    {
                        "label": "Vigentes",
                        "value": status_counts[Contract.Status.ACTIVE],
                        "class": "success",
                    },
                    {
                        "label": "Vencendo em 90 dias",
                        "value": status_counts[Contract.Status.EXPIRING],
                        "class": "warning",
                    },
                    {
                        "label": "Vencidos",
                        "value": status_counts[Contract.Status.EXPIRED],
                        "class": "danger",
                    },
                    {
                        "label": "Encerrados",
                        "value": status_counts[Contract.Status.CLOSED],
                        "class": "neutral",
                    },
                    {
                        "label": "Sem data/rascunho",
                        "value": status_counts[Contract.Status.DRAFT],
                        "class": "neutral",
                    },
                ],
            }
...
class ContractListView(SearchableListView):
    ...
    def get_queryset(self):
        queryset = (
            super()
            .get_queryset()
            .select_related("supplier", "manager", "reference_organization")
        )
        ...
        if status:
            ids = [
                ...
                if contract.calculated_status == status
            ]
...
class FormMetaMixin:
    ...
    def get_cancel_url(self):
        contract_id = self.request.GET.get("contract") or getattr(
            getattr(self, "object", None), "contract_id", None
        )
        if contract_id:
...
class ContractCreateView(
    LoginRequiredMixin, ManagerRequiredMixin, FormMetaMixin, CreateView
):
    model = Contract
    ...
    def get_context_data(self, **kwargs):
        ...
        context["procurement_summary_url_template"] = reverse(
            "procurement_summary", args=[0]
        ).replace("/0/", "/__id__/")
        return context
...
class ContractUpdateView(
    LoginRequiredMixin, ManagerRequiredMixin, FormMetaMixin, UpdateView
):
    model = Contract
    form_class = ContractForm
    template_name = "contracts/form.html"
...
class ProcurementDetailView(LoginRequiredMixin, DetailView):
    ...
    def get_queryset(self):
        return Procurement.objects.select_related(
            ...
            "items__delivery_locations__destination",
            "contracts__supplier",
            "contracts__items__origin_procurement_item",
        )
    ...
    def get_context_data(self, **kwargs):
        ...
        context["procurement_items"] = sorted(
            items, key=lambda item: natural_key(item.item_number)
        )
...
class ProcurementSummaryView(LoginRequiredMixin, View):
    def get(self, request, pk):
        procurement = get_object_or_404(
            Procurement.objects.prefetch_related(
                "items__delivery_locations__destination"
            ),
            pk=pk,
        )
...
class ContractOrderItemsView(LoginRequiredMixin, View):
    def get(self, request, pk):
        ...
        for item in queryset:
            ...
            refs = ", ".join(
                f"{acronym} ({quantity})" for acronym, quantity in locations
            )
            ...
            items.append(
                {
                    ...
                    "locationsText": refs,
                    "procurementItemQuantity": (
                        str(procurement_item_quantity)
                        if procurement_item_quantity
                        else None
                    ),
                }
            )
...
class ProcurementItemFormMixin(LoginRequiredMixin, EditorRequiredMixin, FormMetaMixin):
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.request.method == "POST":
            context["delivery_formset"] = ProcurementItemDeliveryFormSet(
        ...
        context["planned_quantity"] = getattr(
            self.object, "delivered_locations_quantity", Decimal("0")
        )
        return context
        ...
        if formset.is_valid():
            ...
            planned = sum(
                (
                    ...
                    if delivery_form.cleaned_data
                    and not delivery_form.cleaned_data.get("DELETE")
                ),
                Decimal("0"),
            )
            if planned > quantity:
...
class OrganizationListView(SearchableListView):
    ...
    search_fields = ["acronym", "name", "cnpj", "city", "state", "cep", "address"]
...
class SupplyOrderListView(SearchableListView):
    ...
    def get_queryset(self):
        return (
            super()
            .get_queryset()
            .select_related("contract", "destination", "item", "commitment")
...
class DeliveryListView(SearchableListView):
    ...
    search_fields = [
        "supply_order__contract__number",
        "supply_order__destination__acronym",
        "invoice_number",
    ]
    ...
    def get_queryset(self):
        return (
            ...
            .get_queryset()
            .select_related(
                "supply_order__contract", "supply_order__destination", "accepted_by"
            )
        )
...
class SupplierCreateView(
    LoginRequiredMixin, EditorRequiredMixin, FormMetaMixin, CreateView
):
    model = Supplier
...
class SupplierUpdateView(
    LoginRequiredMixin, EditorRequiredMixin, FormMetaMixin, UpdateView
):
    model = Supplier
    form_class = SupplierForm
    template_name = "contracts/form.html"
...
class OrganizationCreateView(
    ...
    template_name = "contracts/form.html"
    title = "Nova Organização Militar"
...
class OrganizationUpdateView(
    ...
    form_class = OrganizationForm
    template_name = "contracts/form.html"
    title = "Editar Organização Militar"
    success_url = reverse_lazy("organization_list")
...
class RelatedCreateView(
    LoginRequiredMixin,
    EditorRequiredMixin,
    PrefillContractMixin,
    FormMetaMixin,
    CreateView,
):
    template_name = "contracts/form.html"
...
class ItemCreateView(RelatedCreateView):
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context["procurement_item_data_url_template"] = reverse(
            "procurement_item_data", args=[0]
        ).replace("/0/", "/__id__/")
        return context
...
class CommitmentCreateView(RelatedCreateView):
    model = Commitment
...
class SupplyOrderCreateView(RelatedCreateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["order_items_url_template"] = reverse(
            "contract_order_items", args=[0]
        ).replace("/0/", "/__id__/")
        return context
...
class DeliveryCreateView(RelatedCreateView):
    model = Delivery
    form_class = DeliveryForm
    title = "Registrar entrega"


class DeliveryUpdateView(RelatedUpdateView):
    model = Delivery
    form_class = DeliveryForm
    title = "Editar entrega"


class ChangeCreateView(RelatedCreateView):
    model = ContractChange
    form_class = ContractChangeForm
    title = "Nova alteração contratual"


class ChangeUpdateView(RelatedUpdateView):
    model = ContractChange
    form_class = ContractChangeForm
    title = "Editar alteração contratual"


class ProcessCreateView(RelatedCreateView):
    model = AdministrativeProcess
    form_class = AdministrativeProcessForm
    title = "Novo PAAI/processo administrativo"


class ProcessUpdateView(RelatedUpdateView):
    model = AdministrativeProcess
    form_class = AdministrativeProcessForm
    title = "Editar PAAI/processo administrativo"
...
class ItemDeleteView(RelatedDeleteView):
    model = ContractItem


class CommitmentDeleteView(RelatedDeleteView):
    model = Commitment


class SupplyOrderDeleteView(RelatedDeleteView):
    model = SupplyOrder


class DeliveryDeleteView(RelatedDeleteView):
    model = Delivery


class ChangeDeleteView(RelatedDeleteView):
    model = ContractChange


class ProcessDeleteView(RelatedDeleteView):
    model = AdministrativeProcess


class DocumentDeleteView(RelatedDeleteView):
    model = Document
...
class ImportPreviewView(LoginRequiredMixin, ManagerRequiredMixin, View):
    ...
    def get(self, request, pk):
        ...
        return render(
            request, self.template_name, {"batch": batch, "preview": batch.preview_data}
        )
    ...
    def post(self, request, pk):
        ...
        if batch.error_count:
            messages.error(
                request,
                "A importação não pode ser confirmada enquanto houver erros impeditivos.",
            )
            return redirect("import_preview", pk=batch.pk)
        try:
            result = import_preview(
                batch.preview_data, actor=request.user, filename=batch.filename
            )
        ...
        return render(
            request, "contracts/import_result.html", {"batch": batch, "result": result}
        )
...
def _contract_report_rows():
    ...
    for contract in Contract.objects.select_related(
        "supplier", "manager", "reference_organization"
    ):
        rows.append(
            [
                contract.number,
                ...
                (
                    contract.reference_organization.acronym
                    if contract.reference_organization
                    else ""
                ),
...
def _procurement_report_rows():
    rows = []
    for procurement in Procurement.objects.select_related(
        "requesting_organization"
        ...
        rows.append(
            [
                ...
                "",
                (
                    procurement.requesting_organization.acronym
                    if procurement.requesting_organization
                    else ""
                ),
...
class ExportContractsXlsxView(LoginRequiredMixin, View):
    def get(self, request):
        content = write_simple_xlsx(
            REPORT_HEADERS, _contract_report_rows(), "Contratos"
        )
...
class DownloadImportTemplateView(LoginRequiredMixin, View):
    def get(self, request):
        headers = [
            ...
            "VALOR UNITÁRIO",
            "VALOR TOTAL",
            "OM TERMO DE REFERÊNCIA",
            "OM DESTINO",
            "GESTOR",
            "SUPLENTE",
            "ASSINATURA DO CONTRATO",
            "VIGÊNCIA FINAL CONTRATO",
            "ASSINATURA DA ORD. FORNECIMENTO",
            "PRAZO DE ENTREGA",
            "OFÍCIO ORDEM FORNECIMENTO À OM",
            "ENTREGUES?",
            "VIATURAS ENTREGUES EM:",
            "STATUS VIGÊNCIA",
            "DIAS P/ VENCER",
            "STATUS ENTREGA",
        ]
        ...
        response["Content-Disposition"] = (
            'attachment; filename="modelo_importacao_sdap.xlsx"'
        )
        return response
...
class ExportContractsPdfView(LoginRequiredMixin, View):
    def get(self, request):
        ...
        elements = [
            Paragraph("Relatório de Contratos — SDAP", styles["Title"]),
            Spacer(1, 5 * mm),
        ]
        data = [
            [
        ...
        for row in _contract_report_rows():
            data.append(
                [
                    ...
                    (
                        "R$ "
                        + f"{row[9]:,.2f}".replace(",", "X")
                        .replace(".", ",")
                        .replace("X", ".")
                    ),
                    (
                        "R$ "
                        + f"{row[11]:,.2f}".replace(",", "X")
                        .replace(".", ",")
                        .replace("X", ".")
                    ),
                ]
        ...
        table = Table(
            data,
            repeatRows=1,
            colWidths=[34 * mm, 78 * mm, 27 * mm, 27 * mm, 16 * mm, 37 * mm, 37 * mm],
        )
        table.setStyle(
            TableStyle(
                [
...
class ExportProcurementsXlsxView(LoginRequiredMixin, View):
    def get(self, request):
        content = write_simple_xlsx(
            PROCUREMENT_REPORT_HEADERS, _procurement_report_rows(), "Pregoes"
        )
        response = HttpResponse(
            content,
            content_type="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
...
class ExportProcurementsPdfView(LoginRequiredMixin, View):
    def get(self, request):
        ...
        styles = getSampleStyleSheet()
        elements = [
            Paragraph("Relatório de Pregões — SDAP", styles["Title"]),
            Spacer(1, 5 * mm),
        ]
        data = [
            [
        ...
        table = Table(
            data,
            repeatRows=1,
            colWidths=[40 * mm, 42 * mm, 35 * mm, 28 * mm, 20 * mm, 40 * mm],
        )
        table.setStyle(
            TableStyle(
                [
